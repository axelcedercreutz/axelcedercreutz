...
for (const [label, body, reason] of [
  ["honeypot filled", good({ website: "https://cheap-seo.example" }), "honeypot"],
  ["no token", good({ token: undefined }), "no-or-forged-token"],
  ["forged token", good({ token: `${T0}.not-the-signature` }), "no-or-forged-token"],
  ["token signed with another secret", good({ token: issueToken("other", T0) }), "no-or-forged-token"],
  ["submitted too fast", good({ token: issueToken(SECRET, T0 + 60_000 - (MIN_FILL_MS - 500)) }), "too-fast"],
  ["link-stuffed", good({ message: "Buy now https://a.example https://b.example https://c.example www.d.example" }), "link-stuffed"],
  ...
  test(`sinkhole: ${label} looks sent but is dropped`, async () => {
    ...
    assert.ok(logs.some((l) => l.includes(reason)), `logged as ${reason}: ${logs}`);
  });
}
